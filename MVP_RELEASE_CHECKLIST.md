...
- [ ] Приложение не показывает blank screen после деплоя
- [ ] При runtime crash работает ErrorBoundary
- [ ] В UI нет mojibake или битой кириллицы
- [ ] Dashboard открывается после деплоя
- [ ] History открывается после деплоя
- [ ] OwnerPanel открывается после деплоя
- [ ] Profile открывается после деплоя
- [ ] Onboarding checklist временно отключён и не считается обязательной фичей
