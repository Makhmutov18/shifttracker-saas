...
import { Building2, CalendarClock, ClipboardCheck, History, LayoutDashboard, LogOut, Menu, Moon, Sun, Users, X } from 'lucide-react';
...
export function Sidebar({ route, user, venueLabel, theme, onTheme, onNavigate, onLogout, open, onClose }: { route: RoutePath; user: User; venueLabel: string; theme: 'light' | 'dark'; onTheme: () => void; onNavigate: (path: RoutePath) => void; onLogout: () => void; open: boolean; onClose: () => void }) {
  return <><div className={`sidebar-backdrop ${open ? 'visible' : ''}`} onClick={onClose} /><aside className={`sidebar ${open ? 'open' : ''}`}>
    ...
    <div className="sidebar-footer">
      ...
      <div className="user-summary"><div className="avatar">{(user.name || 'П').slice(0, 1).toUpperCase()}</div><div><strong>{user.name || 'Пользователь'}</strong><span>{roleLabels[user.role] ?? 'Сотрудник'} · {venueLabel}</span></div><button className="logout-button" onClick={onLogout}><LogOut />Выйти</button></div>
    </div>
  </aside></>;
}
...
export function AppShell({ route, user, venues, venueId, onVenue, theme, onTheme, onNavigate, onLogout, children, title, action }: { route: RoutePath; user: User; venues: Venue[]; venueId: string; onVenue: (value: string) => void; theme: 'light' | 'dark'; onTheme: () => void; onNavigate: (path: RoutePath) => void; onLogout: () => void; children: ReactNode; title: string; action?: ReactNode }) {
  const [menuOpen, setMenuOpen] = React.useState(false);
  const venueLabel = venueId ? venues.find((venue) => venue.id === venueId)?.name ?? 'Точка' : 'Все точки';
  return <div className="app-shell"><Sidebar route={route} user={user} venueLabel={venueLabel} theme={theme} onTheme={onTheme} onNavigate={onNavigate} onLogout={onLogout} open={menuOpen} onClose={() => setMenuOpen(false)} /><main><TopBar title={title} venues={venues} venueId={venueId} onVenue={onVenue} user={user} onMenu={() => setMenuOpen(true)} action={action} /><div className="content">{children}</div></main></div>;
}
