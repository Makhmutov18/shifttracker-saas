...
import { api } from '../api';
import { AvatarStack, Badge, ConfirmationDialog, DataTable, Drawer, EmptyState, ErrorState, FormField, LoadingState, MoneyValue, PageHeader, Pagination, Sparkline, Toast, type SortDirection } from '../components/ui';
...
export function VenuesPage() {
  ...
  const stats = useMemo(() => new Map((venues ?? []).map((venue) => {
    ...
    const revenueSeries = Array.from({ length: days }, (_, index) => venueShifts.filter((shift) => Number((shift.date || '').slice(8, 10)) === index + 1).reduce((sum, shift) => sum + Number(shift.revenue || 0), 0));
    return [venue.id, { employees: venueEmployees.length, employeeNames: venueEmployees.map((employee) => employee.name || 'Сотрудник'), shifts: venueShifts.length, revenue: venueShifts.reduce((sum, shift) => sum + Number(shift.revenue || 0), 0), revenueSeries }];
  })), [venues, shifts, users]);
  const filtered = useMemo(() => (venues ?? []).filter((venue) => venue.is_active !== archive).sort((left, right) => {
    const leftStats = stats.get(left.id) || { employees: 0, employeeNames: [], shifts: 0, revenue: 0, revenueSeries: [] };
    const rightStats = stats.get(right.id) || { employees: 0, employeeNames: [], shifts: 0, revenue: 0, revenueSeries: [] };
  ...
  return <>
    ...
    <section className="panel"><DataTable label="Список точек" headers={[{ label: 'Точка', sortKey: 'name' }, { label: 'Статус', sortKey: 'status' }, { label: 'Сотрудников', sortKey: 'employees' }, { label: 'Смен за месяц', sortKey: 'shifts' }, { label: 'Выручка за месяц', sortKey: 'revenue', align: 'right' }, '']} sortKey={sortKey} sortDirection={sortDirection} onSort={toggleSort} empty={!filtered.length}>
      {pageRows.map((venue) => { const venueStats = stats.get(venue.id) || { employees: 0, employeeNames: [], shifts: 0, revenue: 0, revenueSeries: [] }; return <tr key={venue.id}><td><div className="venue-name-cell"><span><Building2 /></span><strong>{venue.name || 'Точка без названия'}</strong></div></td><td><Badge variant={venue.is_active ? 'success' : 'neutral'}>{venue.is_active ? 'Активна' : 'В архиве'}</Badge></td><td><div className="venue-team-cell"><AvatarStack items={venueStats.employeeNames.map((name) => ({ name }))} max={3} /><span>{venueStats.employees}</span></div></td><td>{venueStats.shifts}</td><td className="align-right"><div className="venue-revenue-cell"><MoneyValue value={venueStats.revenue} muted={!venueStats.shifts} /><Sparkline values={venueStats.revenueSeries} label={`Выручка за месяц: ${venue.name}`} /></div></td><td><div className="row-actions"><button className="icon-button" onClick={() => open(venue)} aria-label="Редактировать точку"><Pencil /></button>{venue.is_active ? <button className="icon-button danger-icon" onClick={() => setConfirm(venue)} aria-label="В архив"><Archive /></button> : <button className="icon-button" onClick={() => void restore(venue)} aria-label="Восстановить"><RotateCcw /></button>}</div></td></tr>; })}
    ...
    <div className="mobile-cards">{pageRows.map((venue) => { const venueStats = stats.get(venue.id) || { employees: 0, employeeNames: [], shifts: 0, revenue: 0, revenueSeries: [] }; return <div className="mobile-card" key={venue.id}><strong>{venue.name || 'Точка без названия'}</strong><p>{venueStats.employees} сотрудников · {venueStats.shifts} смен</p><div className="mobile-venue-stat"><MoneyValue value={venueStats.revenue} muted={!venueStats.shifts} /><Sparkline values={venueStats.revenueSeries} label={`Выручка за месяц: ${venue.name}`} /></div><button className="button secondary" onClick={() => open(venue)}>Редактировать</button></div>; })}</div>
