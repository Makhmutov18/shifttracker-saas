...
export function EmployeesPage({ user, venues, venueId }: { user: User; venues: Venue[]; venueId: string }) {
  ...
  const save = async () => {
    if (!form.name.trim() || !form.venue_id) { setError('Укажите имя и основную точку сотрудника.'); return; }
  ...
  return <div className="employees-page">
    ...
    <section className="panel">
      <DataTable label="Список сотрудников" headers={[{ label: 'Сотрудник', sortKey: 'name' }, { label: 'Роль', sortKey: 'role' }, { label: 'Основная точка', sortKey: 'venue' }, { label: 'Модель оплаты', sortKey: 'pay' }, 'Ставка / процент', 'Доступ', 'Действия']} sortKey={sortKey} sortDirection={sortDirection} onSort={toggleSort} empty={!filtered.length}>
        {pageRows.map((employee) => <tr className="employee-table-row" tabIndex={0} onClick={() => setPreviewEmployee(employee)} onKeyDown={(event) => openFromKeyboard(event, employee)} key={employee.id}><td><div className="person-cell"><AvatarStack items={[{ name: employee.name || 'Сотрудник' }]} max={1} /><span><strong>{employee.name || 'Сотрудник'}</strong><small>{employee.position || 'Без должности'}</small></span></div></td><td>{roleLabels[employee.role] || 'Сотрудник'}</td><td>{employee.venue?.name || venueMap.get(employee.venue_id || '') || 'Основная точка не указана'}</td><td><Badge variant="neutral" icon={<WalletCards />}>{payModelLabels[employee.pay_model] || 'Не настроена'}</Badge></td><td className="employee-pay-value">{formatEmployeePay(employee)}</td><td>{hasManagementAccess(employee) ? 'Есть доступ' : 'Обычный сотрудник'}</td><td><div className="row-actions" onClick={(event) => event.stopPropagation()} onKeyDown={(event) => event.stopPropagation()}><button className="icon-button" onClick={() => openEdit(employee)} aria-label="Редактировать"><Pencil /></button>{employee.is_active ? <button className="icon-button danger-icon" onClick={() => setConfirmUser(employee)} aria-label="В архив"><Archive /></button> : <button className="icon-button" onClick={() => void restore(employee)} aria-label="Восстановить"><RotateCcw /></button>}</div></td></tr>)}
      </DataTable>
      ...
      <div className="mobile-cards employee-mobile-cards">{pageRows.map((employee) => <article className="mobile-card employee-mobile-card" role="button" tabIndex={0} onClick={() => setPreviewEmployee(employee)} onKeyDown={(event) => openFromKeyboard(event, employee)} key={employee.id}><div className="employee-mobile-head"><AvatarStack items={[{ name: employee.name || 'Сотрудник' }]} max={1} /><span><strong>{employee.name || 'Сотрудник'}</strong><small>{employee.position || 'Без должности'}</small></span><Badge variant="neutral">{roleLabels[employee.role] || 'Сотрудник'}</Badge></div><div className="employee-mobile-details"><span><small>Основная точка</small><strong>{employee.venue?.name || venueMap.get(employee.venue_id || '') || 'Основная точка не указана'}</strong></span><span><small>Оплата</small><strong>{payModelLabels[employee.pay_model] || 'Не настроена'} · {formatEmployeePay(employee)}</strong></span><span><small>Доступ</small><strong>{hasManagementAccess(employee) ? 'Есть доступ' : 'Обычный сотрудник'}</strong></span></div><div className="employee-mobile-actions" onClick={(event) => event.stopPropagation()} onKeyDown={(event) => event.stopPropagation()}><button className="icon-button" onClick={() => openEdit(employee)} aria-label="Редактировать"><Pencil /></button>{employee.is_active ? <button className="icon-button danger-icon" onClick={() => setConfirmUser(employee)} aria-label="В архив"><Archive /></button> : <button className="icon-button" onClick={() => void restore(employee)} aria-label="Восстановить"><RotateCcw /></button>}</div></article>)}</div>
    ...
    <Drawer title="Карточка сотрудника" open={Boolean(previewEmployee)} onClose={() => setPreviewEmployee(null)} footer={previewEmployee ? previewEmployee.is_active ? <button className="button primary" onClick={() => { const employee = previewEmployee; setPreviewEmployee(null); openEdit(employee); }}><Pencil />Редактировать</button> : <button className="button primary" disabled={saving} onClick={() => { const employee = previewEmployee; setPreviewEmployee(null); void restore(employee); }}><RotateCcw />Восстановить</button> : undefined}>
      {previewEmployee && <div className="employee-profile"><div className="employee-profile-head"><span className="avatar large">{(previewEmployee.name || 'С').slice(0, 1)}</span><div><h3>{previewEmployee.name || 'Сотрудник'}</h3><p>{previewEmployee.position || 'Без должности'}</p></div><Badge variant={previewEmployee.is_active ? 'success' : 'neutral'}>{previewEmployee.is_active ? 'Активен' : 'В архиве'}</Badge></div><dl><div><dt>Роль</dt><dd>{roleLabels[previewEmployee.role] || 'Сотрудник'}</dd></div><div><dt>Основная точка</dt><dd>{previewEmployee.venue?.name || venueMap.get(previewEmployee.venue_id || '') || 'Основная точка не указана'}</dd></div><div><dt>Модель оплаты</dt><dd>{payModelLabels[previewEmployee.pay_model] || 'Оплата не настроена'}</dd></div><div><dt>Условия оплаты</dt><dd>{formatEmployeePay(previewEmployee)}</dd></div><div><dt>Управленческий доступ</dt><dd>{hasManagementAccess(previewEmployee) ? 'Есть' : 'Нет'}</dd></div></dl></div>}
    </Drawer>
